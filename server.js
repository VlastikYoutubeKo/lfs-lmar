// LFS InSim Live Map + Radio Server - PRODUCTION VERSION (CommonJS)
// Usage: node server.js [--cop] [--debug] [--map-logger]

const { InSim } = require('node-insim');
const { WebSocketServer } = require('ws');
const { 
  PacketType, InSimFlags, ButtonStyle, 
  IS_BTN, IS_BFN, IS_TINY, IS_MSO, 
  TinyType, UserType, IS_ISI_ReqI 
...
let getTrackName, initRadioBrowser, searchStationsByName, voteForStation;
let getAbradiaStations, prepareAbradiaStation, getAbradiaNowPlaying;
let parseMissionDestination, getDestinationCoords, calculateDistance, calculateETA, formatTime;
let loadExistingLocations, processApiData, setupGracefulShutdown, printStats;

// Load ES modules dynamically
(async () => {
  const trackNamesModule = await import('./track_names.js');
  getTrackName = trackNamesModule.getTrackName;
  
  const radioBrowserModule = await import('./radio_browser.js');
  initRadioBrowser = radioBrowserModule.initRadioBrowser;
  searchStationsByName = radioBrowserModule.searchStationsByName;
  voteForStation = radioBrowserModule.voteForStation;
  
  const abradiaModule = await import('./abradia_api.js');
  getAbradiaStations = abradiaModule.getAbradiaStations;
  prepareAbradiaStation = abradiaModule.prepareAbradiaStation;
  getAbradiaNowPlaying = abradiaModule.getAbradiaNowPlaying;
  
  const tcLocationsModule = await import('./tc_locations.js');
  parseMissionDestination = tcLocationsModule.parseMissionDestination;
  getDestinationCoords = tcLocationsModule.getDestinationCoords;
  calculateDistance = tcLocationsModule.calculateDistance;
  calculateETA = tcLocationsModule.calculateETA;
  formatTime = tcLocationsModule.formatTime;
  
  if (ENABLE_GPS_LOGGER) {
    const gpsLoggerModule = await import('./gps_logger.js');
    loadExistingLocations = gpsLoggerModule.loadExistingLocations;
    processApiData = gpsLoggerModule.processApiData;
    setupGracefulShutdown = gpsLoggerModule.setupGracefulShutdown;
    printStats = gpsLoggerModule.printStats;
  }
  
  // Start server after modules loaded
  startServer();
})();

// === SWITCHES ===
const FORCE_COP_MODE = process.argv.includes('--cop');
const ENABLE_DEBUG = process.argv.includes('--debug');
const ENABLE_GPS_LOGGER = process.argv.includes('--map-logger');
...
async function updateChaseData() {
    ...
    try {
        ...
        if (ENABLE_GPS_LOGGER && processApiData) {
          processApiData(data, MY_UCID);
        }
        ...
        if (data.ms && Array.isArray(data.ms)) {
            data.ms.forEach(m => {
                ...
                if (text.includes("Barrier Fixing") || text.includes("Pothole Fixing") || text.includes("Traffic Light")) {
                    const locName = parseMissionDestination ? parseMissionDestination(text) : null;
                    const coords = getDestinationCoords ? getDestinationCoords(locName) : null;
        ...
        if (data.ps && typeof data.ps === 'object') {
            Object.values(data.ps).forEach(player => {
                ...
                if (data.ms) {
                    ...
                    if (mission) {
                        if (missionData) {
                            ...
                            const parsed = parseMissionDestination ? parseMissionDestination(mission.lt) : null;
                            if (parsed) missionData.destination = parsed;
                        } else {
                            const parsedDest = parseMissionDestination ? parseMissionDestination(mission.lt) : null;
                            const coords = getDestinationCoords ? getDestinationCoords(parsedDest) : null;
                            if (coords) {
                                missionData = {
                ...
                if (missionData && calculateDistance && calculateETA) {
...
let server, wssMap, radioServer, wssRadio;
let inSim, cars, currentTrack, currentLayout;

function startServer() {
  if (ENABLE_GPS_LOGGER && loadExistingLocations) {
    loadExistingLocations();
    if (setupGracefulShutdown) setupGracefulShutdown();
    if (printStats) setInterval(printStats, 60000);
  }
  ...
  wssMap = new WebSocketServer({ server });
  ...
  radioServer = http.createServer();
  wssRadio = new WebSocketServer({ server: radioServer });
  ...
  if (initRadioBrowser) initRadioBrowser();
}

// === INSIM HANDLERS ===
function setupInSimHandlers() {
  inSim.connect({
    Host: '127.0.0.1',
    Port: 29999,
    IName: 'LiveMap',
    Flags: InSimFlags.ISF_MCI | InSimFlags.ISF_LOCAL | InSimFlags.ISF_MSO,
  ...
  inSim.on(PacketType.ISP_STA, (p) => {
    ...
    if (newTrack && newTrack !== currentTrack) {
      ...
      const trackName = getTrackName ? getTrackName(currentTrack) : currentTrack;
  ...
  inSim.on(PacketType.ISP_BTC, async (p) => {
      ...
      if (p.ClickID === 239) renderUI(MY_UCID, 'main');
      ...
      else if (p.ClickID === 212 && getAbradiaStations) renderUI(MY_UCID, 'abradia', await getAbradiaStations(), 0);
          ...
          if (station) {
              if ((station.provider === 'abradia' || station.slug) && prepareAbradiaStation) {
                  ...
                  if (station.id && voteForStation) voteForStation(station.id);
              }
          }
      }
  ...
  inSim.on(PacketType.ISP_BTT, async (p) => {
      if (p.UCID === MY_UCID && p.ClickID === 221 && p.Text && searchStationsByName) {
...
async function playRadioStream(url, name, stationConfig = null) {
  ...
  if (MPV_PATH) {
      ...
      if (stationConfig?.provider === 'abradia' && stationConfig?.slug && getAbradiaNowPlaying) {
        startAbradiaMetadataFetch();
      }
...
async function fetchAbradiaMetadata() {
  if (!currentStationConfig?.slug || !getAbradiaNowPlaying) return;
  try {
